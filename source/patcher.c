...
int patch_code(u64 progid, u8 *code, u32 size)
{
  for(int i = 0; i < patches_count; i++) {
    if (patches_list[i].titleid == progid) {
      // Matched the TitleID. Apply.
      patch_memory(code, size,
                   patches_list[i].pattern, patches_list[i].pattern_size, patches_list[i].offset,
                   patches_list[i].patch,   patches_list[i].patch_size,   patches_list[i].count);
    }
  }
